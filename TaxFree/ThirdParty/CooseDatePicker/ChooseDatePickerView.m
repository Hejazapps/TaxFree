...
@property (nonatomic, strong) NSDateComponents *maxDateComp;
...
- (void)awakeFromNib {
    ...
    NSDate *date = [dateFormatter dateFromString:@"2050-12-31"];
    self.maxDateComp = [calendar components: unitFlags fromDate:date];
    
    yearIndex = [self.yearArray indexOfObject:[NSString stringWithFormat:@"%ld年", self.maxDateComp.year]];
    monthIndex = [self.monthArray indexOfObject:[NSString stringWithFormat:@"%02ld月", self.maxDateComp.month]];
    dayIndex = [self.dayArray indexOfObject:[NSString stringWithFormat:@"%02ld日", self.maxDateComp.day]];
...
- (void)setMaxDate:(NSDate *)maxDate {
    ...
    unsigned unitFlags = NSCalendarUnitYear | NSCalendarUnitMonth |  NSCalendarUnitDay | NSCalendarUnitHour |  NSCalendarUnitMinute | NSCalendarUnitSecond | NSCalendarUnitWeekday;
    // 获取不同时间字段的信息
    self.maxDateComp = [calendar components: unitFlags fromDate:maxDate];
...
//    if (_yearArray == nil) {
        ...
        for (int year = 1900; year <= self.maxDateComp.year; year++) {
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component {
    if (component == 0) {
        ...
        if ([self.yearArray[yearIndex] isEqualToString:self.yearArray.lastObject]) {
            return self.maxDateComp.month;
        }
        return self.monthArray.count;
    } else {
        // 如果是今年今月返回当前已过天数
        if ([self.yearArray[yearIndex] isEqualToString:self.yearArray.lastObject] && self.maxDateComp.month - 1 == monthIndex) {
            return self.maxDateComp.day;
        }
        switch (monthIndex + 1) {
            case 2:{
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    if (component == 0) {
        ...
        if ([self.yearArray[row] isEqualToString:self.yearArray.lastObject]) {
            if (self.maxDateComp.month - 1 < monthIndex) {
                monthIndex = self.maxDateComp.month - 1;
                [self selectActionToPickerView:pickerView row:monthIndex inComponent:1];
                
                dayIndex = self.maxDateComp.day - 1;
                [self selectActionToPickerView:pickerView row:dayIndex inComponent:2];
            }
            
            if (self.maxDateComp.month - 1 == monthIndex && self.maxDateComp.day - 1 < dayIndex) {
                dayIndex = self.maxDateComp.day - 1;
                [self selectActionToPickerView:pickerView row:dayIndex inComponent:2];
            }
        }
        ...
        if ([pickerYear isEqualToString:self.yearArray.lastObject] && self.maxDateComp.month - 1 < monthIndex) {
            monthIndex = self.maxDateComp.month - 1;
            dayIndex = self.maxDateComp.day - 1;
            [self selectActionToPickerView:pickerView row:monthIndex inComponent:1];
            [self selectActionToPickerView:pickerView row:dayIndex inComponent:2];
        ...
        if ([pickerYear isEqualToString:self.yearArray.lastObject] && [pickerMonth isEqualToString:self.monthArray[self.maxDateComp.month - 1]] && self.maxDateComp.day - 1 < dayIndex) {
            dayIndex = self.maxDateComp.day - 1;
            [self selectActionToPickerView:pickerView row:dayIndex inComponent:2];
        }
        [pickerView selectRow:dayIndex inComponent:2 animated:YES];
    ...
    NSString *timeStr = [NSString stringWithFormat:@"%@%@%@", self.yearArray[yearIndex], self.monthArray[monthIndex], self.dayArray[dayIndex]];
    ...
    if (_isExpiry) {
        if ([[formatter dateFromString:[formatter stringFromDate:[NSDate date]]] timeIntervalSince1970] > [finalDate timeIntervalSince1970]) {
